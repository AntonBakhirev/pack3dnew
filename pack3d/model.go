...
func CreateRotations(lx bool, ly bool, lz bool, dax int, day int, daz int) {
	aZ := AxisZ.Vector()
	aY := AxisY.Vector()
	aX := AxisX.Vector()
	for i := 0; i < 360; i += daz {
		for j := 0; j < 360; j += day {
			for k := 0; k < 360; k += dax {
				md := fauxgl.Rotate(aZ, float64(i)*fauxgl.Radians(float64(daz)))
				md = md.Rotate(aY, float64(j)*fauxgl.Radians(float64(day)))
				md = md.Rotate(aX, float64(k)*fauxgl.Radians(float64(dax)))

				Rotations = append(Rotations, md)
			}
		}
	}
}
...
func (m *Model) Pack(iterations int, lx bool, ly bool, lz bool, ax int, ay int, az int, callback AnnealCallback) *Model {
	e := 0.5
	return Anneal(m, 1e0*e, 1e-4*e, iterations, lx, ly, lz, ax, ay, az, callback).(*Model)
}
