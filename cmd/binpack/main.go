...
func init() {
	for i := 0; i < RollCount; i++ {
		m_i := fauxgl.Rotate(fauxgl.Vector{1, 0, 0}, float64(i)*math.Pi*2/RollCount)
		for j := 0; j < RollCount; j++ {
			m_j := m_i
			m_j = m_j.Rotate(fauxgl.Vector{0, 1, 0}, float64(j)*math.Pi*2/RollCount)
			for k := 0; k < RollCount; k++ {
				m_k := m_j
				m_k = m_k.Rotate(fauxgl.Vector{0, 0, 1}, float64(k)*math.Pi*2/RollCount)
				Rotations = append(Rotations, m_k)
			}
		}
	}
}
